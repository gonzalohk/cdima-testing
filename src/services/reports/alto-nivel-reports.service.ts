 ...
 */
export const exportAltoNivelGeneralPDF = ({ curso, docentes, estudiantes }: ExportDiplomadoGeneralParams): void => {
  const colors = {
    black: [0, 0, 0],
    white: [255, 255, 255],
  ...
  pdf.text(`CURSO DE ALTO NIVEL: ${curso.name}`, pageWidth - margins.right, metaY, { align: 'right' });
...
export const exportAltoNivelGeneralWord = ({ curso, estudiantes }: ExportDiplomadoGeneralWordParams): void => {
  ...
  link.href = url;
  link.download = `lista-participantes-alto-nivel-${curso.name.replace(/\s+/g, '-').toLowerCase()}.doc`;
...
export const exportAltoNivelCentralizadorNotasWord = ({ curso, estudiantes, numeroModulos }: ExportDiplomadoCentralizadorNotasParams): void => {
  ...
  const link = document.createElement('a');
  link.href = url;
  link.download = `nomina-oficial-alto-nivel-${curso.name.replace(/\s+/g, '-').toLowerCase()}.doc`;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
  URL.revokeObjectURL(url);
...
export const exportAltoNivelEstudiantePDF = ({ estudiante, curso, numeroModulos }: ExportDiplomadoEstudianteParams): void => {
  ...
  const promedio = moduloValues.reduce((a, b) => a + b, 0) / n;
  ...
  const obs = parseNotasObservaciones(estudiante.notes);
 ...
 */
export const exportAltoNivelEstudianteWord = ({ estudiante, curso, numeroModulos }: ExportDiplomadoEstudianteParams): void => {
  const n = Math.min(10, Math.max(1, Math.round(numeroModulos ?? 5)));
  const escapeHtml = (value: string): string =>
    value
      .replace(/&/g, '&amp;')
  ...
  const modulosConNotas = allModuloKeys.slice(0, n).map((field, i) => ({
    nombre: `Módulo ${i + 1}`,
  ...
  const contextoLabel = curso ? `<div>CURSO DE ALTO NIVEL: ${escapeHtml(curso.name)}</div>` : '';
  const fechaGeneracion = format(new Date(), "dd 'de' MMMM 'de' yyyy", { locale: es });
 ...
 */
export const exportAltoNivelNominaPDF = ({ curso, estudiantes }: { curso: AsanaSection; estudiantes: AsanaTask[] }): void => {
  const colors = {
  ...
  pdf.setFontSize(11);
  pdf.setFont('helvetica', 'normal');
  pdf.text(`PROGRAMA: ${curso.name}`, pageWidth - margins.right, margins.top + 12, { align: 'right' });
...
export const exportAltoNivelNominaWord = ({ curso, estudiantes }: { curso: AsanaSection; estudiantes: AsanaTask[] }): void => {
  const escapeHtml = (value: string): string =>
    value
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      ...
      <body>
        <div class="header-grid">
          ...
          <div class="header-right">
            ...
            <p class="subtitle">PROGRAMA: ${escapeHtml(curso.name)}</p>
  ...
  a.href = url;
  a.download = `Nomina_${curso.name.replace(/[^a-zA-Z0-9]/g, '_')}.doc`;
  a.click();
  URL.revokeObjectURL(url);
};

export const exportAltoNivelAsistenciaPDF = ({ curso, estudiantes }: { curso: AsanaSection; estudiantes: AsanaTask[] }): void => {
  const MIN_COLS = 15;
  const colors = {
    black: [0, 0, 0] as [number, number, number],
    headerGray: [220, 220, 220] as [number, number, number],
...
export const exportAltoNivelAsistenciaWord = ({ curso, estudiantes }: { curso: AsanaSection; estudiantes: AsanaTask[] }): void => {
  const escapeHtml = (value: string): string =>
    value
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `Asistencia_${curso.name.replace(/[^a-zA-Z0-9]/g, '_')}.doc`;
  a.click();
  URL.revokeObjectURL(url);
};
