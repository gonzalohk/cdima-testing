...
export const exportEscuelaCentralizadorNotasPDF = ({
  ...
  const n = Math.min(10, Math.max(1, Math.round(numeroModulos ?? 7)));
  const colors = {
    black: [0, 0, 0],
    white: [255, 255, 255],
    lightGray: [245, 245, 245],
  ...
  const pdf = new jsPDF({
    orientation: n > 7 ? 'landscape' : 'portrait',
    unit: 'mm',
    format: 'letter'
  });
  ...
  if (notasEstudiantes.length > 0) {
    ...
    autoTable(pdf, {
      ...
      columnStyles: {
        ...
        [3 + n]: { cellWidth: 16, halign: 'center' }
      },
      didParseCell: (data: any) => {
        if (data.section === 'head' && data.row.index === 0 && data.column.index >= 3 && data.column.index < 3 + n) {
...
export const exportEscuelaCentralizadorNotasWord = ({
  ...
  const n = Math.min(7, Math.max(1, Math.round(numeroModulos ?? 7)));
  const escapeHtml = (value: string): string =>
    value
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
...
export const exportEscuelaActaCalificacionesWord = ({
  ...
  const filas = notasEstudiantes.map((est, index) => {
    ...
    return `
      <tr>
        <td class="num-cell">${index + 1}</td>
        <td class="num-cell">${escapeHtml(est.ci)}</td>
        <td class="name-cell">${escapeHtml(est.nombreFormateado)}</td>
        ${notas}
        <td class="num-cell">${est.total.toFixed(1)}</td>
        <td class="num-cell">${aprobado ? 'APROBADO' : 'REPROBADO'}</td>
      </tr>
    `;
  }).join('');
    ...
    <html>
      <head>
        ...
        <style>
          ...
          table { width: 100%; border-collapse: collapse; table-layout: fixed; }
          td { border: 1px solid #000; padding: 2px 3px; font-size: 11px; font-weight: normal; }
          ...
          .header-col { display: table-cell; vertical-align: middle; width: 33%; text-align: center; font-size: 10px; font-weight: bold; }
          .header-col-center { display: table-cell; vertical-align: middle; width: 34%; text-align: center; }
          .logo { width: 72px; height: 72px; object-fit: contain; display: block; margin: 0 auto 4px auto; }
          .title { margin: 0; font-size: 16px; font-weight: bold; letter-spacing: 0.4px; }
          .subtitle { margin: 6px 0 0 0; font-size: 14px; font-weight: bold; text-transform: uppercase; }
          .meta { font-size: 11px; font-weight: bold; line-height: 1.8; margin-top: 8px; }
      ...
      <body>
        ...
        <table>
          <tbody>
            <tr class="header-row">
              <td style="width:30px;" class="num-cell">No.</td>
              <td style="width:85px;" class="num-cell">C.I.</td>
              <td style="width:35%;" class="name-cell">NOMBRES Y APELLIDOS</td>
              ${Array.from({ length: n }, (_, i) => `<td class="num-cell">MÓD. ${i + 1}</td>`).join('\n              ')}
              <td class="num-cell">PROMEDIO</td>
              <td style="width:80px;" class="num-cell">RESULTADO</td>
            </tr>
            ${filas || `<tr><td colspan="${n + 5}" class="num-cell">No hay participantes registrados</td></tr>`}
            ${notasEstudiantes.length > 0 ? `
            <tr>
              <td></td>
              <td></td>
              <td class="name-cell"><strong>PROMEDIO GENERAL</strong></td>
              <td colspan="${n}"></td>
              <td class="num-cell"><strong>${promedioGeneral()}</strong></td>
              <td></td>
            </tr>` : ''}
          </tbody>
        </table>
...
export const exportEscuelaEstudiantePDF = ({
  ...
  const modulosConNotas = allModuloFields.slice(0, n).map((field, i) => ({
    nombre: `Módulo ${i + 1}`,
    nota: getCustomFieldValueSafe(estudiante, field, 0),
  }));
  const promedio = modulosConNotas.reduce((sum, m) => sum + m.nota, 0) / n;
  ...
  const notasData = modulosConNotas.map(m => [
    m.nombre, m.nota.toString(), m.nota >= 51 ? 'Aprobado' : 'Reprobado', obsModulos[m.nombre] || ''
  ]);
 ...
 */
export const exportEscuelaEstudianteWord = ({
  estudiante,
  escuela,
  numeroModulos
}: ExportEscuelaEstudianteParams): void => {
  ...
  const contextoLabel = escuela ? `<div>ESCUELA: ${escapeHtml(escuela.name)}</div>` : '';
  const fechaGeneracion = format(new Date(), "dd 'de' MMMM 'de' yyyy", { locale: es });
 ...
 */
export const exportEscuelaNominaPDF = ({
  escuela,
  estudiantes
}: ExportEscuelaGeneralParams): void => {
  const colors = {
  ...
  pdf.setFontSize(11);
  pdf.setFont('helvetica', 'normal');
  pdf.text(`PROGRAMA: ${escuela.name}`, pageWidth - margins.right, margins.top + 12, { align: 'right' });
  ...
  // DATOS: parsear y ordenar alfabéticamente por apellido paterno
...
export const exportEscuelaNominaWord = ({
  escuela,
  estudiantes
}: ExportEscuelaGeneralParams): void => {
  const escapeHtml = (value: string): string =>
      ...
      <body>
        <div class="header-grid">
          ...
          <div class="header-right">
            ...
            <p class="subtitle">PROGRAMA: ${escapeHtml(escuela.name)}</p>
  ...
  a.href = url;
  a.download = `Nomina_${escuela.name.replace(/[^a-zA-Z0-9]/g, '_')}.doc`;
  a.click();
  URL.revokeObjectURL(url);
};

export const exportEscuelaAsistenciaPDF = ({
  escuela,
  estudiantes
}: ExportEscuelaGeneralParams): void => {
  const MIN_COLS = 15;
  ...
  const participantes = estudiantes.map(est => {
    ...
    return {
      gid: est.gid,
      nombreFormateado: `${apellidoPaterno} ${apellidoMaterno} ${nombre}`.trim(),
      apellidoPaterno,
      notes: est.notes,
    };
  }).sort((a, b) => a.apellidoPaterno.localeCompare(b.apellidoPaterno, 'es', { sensitivity: 'base' }));

  // Mapa de asistencia: gid -> fecha -> 'A' | 'F'
  const asistenciaMap = new Map<string, Map<string, string>>();
  estudiantes.forEach(est => {
    const mapaFechas = new Map<string, string>();
    parseAsistenciaRecords(est.notes).forEach(r => {
      if (r.fecha) mapaFechas.set(r.fecha, r.asistio ? 'A' : 'F');
    });
  ...
  const body = participantes.map((p, i) => {
    ...
    return [(i + 1).toString(), p.nombreFormateado, ...fechasCols.map(f => f ? (mapaEst.get(f) ?? '') : '')];
  });
  ...
  const colStyles: Record<number, object> = {
    0: { halign: 'center', cellWidth: numWidth },
    1: { halign: 'left', cellWidth: nombreWidth },
  };
  for (let i = 0; i < numCols; i++) {
    colStyles[i + 2] = { halign: 'center', cellWidth: dateColWidth };
  }
  ...
  autoTable(pdf, {
    ...
    theme: 'grid',
    styles: {
      fontSize: 7.5, cellPadding: 2, textColor: colors.black,
      lineColor: [180, 180, 180] as [number, number, number], lineWidth: 0.2,
...
export const exportEscuelaAsistenciaWord = ({
  escuela,
  estudiantes
}: ExportEscuelaGeneralParams): void => {
  const escapeHtml = (value: string): string =>
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `Asistencia_${escuela.name.replace(/[^a-zA-Z0-9]/g, '_')}.doc`;
  a.click();
  URL.revokeObjectURL(url);
};
