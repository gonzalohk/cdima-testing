...
const ProduccionAltoNivelPage: React.FC = () => {
  ...
  useEffect(() => {
    if (location.state?.selectedCurso && cursos.length > 0) {
      const diplomado = cursos.find(d => d.gid === location.state.selectedCurso.gid);
      if (diplomado) {
  ...
  const loadCursos = async () => {
    ...
    try {
      ...
      // Obtener proyectos y buscar "Curso Alto Nivel CDIMA"
      const projects = await asanaService.getProjects(cdima.gid);
      const cursosProject = projects.find(p => 
        p.name.toLowerCase().includes('curso alto nivel')
      );

      if (!cursosProject) {
        throw new Error('No se encontró el proyecto "Curso Alto Nivel CDIMA"');
      }

      setCursosProjectGid(cursosProject.gid);

      // Obtener secciones del proyecto (cada sección es un curso)
      const sections = await asanaService.getSections(cursosProject.gid);
      setCursos(sections);
    } catch (err) {
  ...
  const handleViewDetails = async (curso: AsanaSection) => {
    ...
    try {
      ...
      const sectionTasks = await asanaService.getSectionTasks(curso.gid);
      ...
      console.error('Error loading curso details:', err);
  ...
  const handleGuardarNotas = async (soloReintentar: boolean = false) => {
    ...
    try {
      ...
      if (fallidos.length > 0) {
        ...
        if (selectedCurso) {
          await handleViewDetails(selectedCurso);
        }
  ...
  const handleExportDiplomadoGeneral = async () => {
    if (!selectedCurso) return;
    try {
      await exportAltoNivelGeneralPDF({
        curso: selectedCurso,
        docentes,
        estudiantes
      });
  ...
  const handleExportDiplomadoGeneralWord = async () => {
    if (!selectedCurso) return;
    try {
      await exportAltoNivelGeneralWord({
        curso: selectedCurso,
        estudiantes
      });
    } catch (error) {
  ...
  const handleEliminarSeccion = async () => {
    ...
    try {
      ...
      await asanaService.deleteSection(selectedCurso.gid);
      ...
      setNotification({ message: err instanceof Error ? err.message : 'Error al eliminar curso', type: 'error' });
    } finally {
    ...
    <div className="planning-page">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.875rem 0', marginBottom: '0', gap: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', minWidth: 0, flex: 1 }}>
          ...
          <span style={{ fontSize: '1rem', color: '#64748b', fontWeight: 600, whiteSpace: 'nowrap', flexShrink: 0, letterSpacing: '-0.01em' }}>Curso en:</span>
          <style>{`
          ...
          <Select
            ...
            size="large"
            value={selectedCurso?.gid || undefined}
            onChange={(value) => {
              if (!value) { setSelectedCurso(null); return; }
              const found = cursos.find(d => d.gid === value);
              if (found) handleViewDetails(found);
            }}
            options={cursos.map(d => ({ label: d.name, value: d.gid }))}
            placeholder={loading ? 'Cargando...' : 'Seleccionar curso...'}
            loading={loading}
            allowClear
            showSearch
          ...
          <Dropdown
            menu={{ items: [
              ...
              { type: 'divider' },
              {
                ...
                label: (
                  <span title={docentes.length > 0 || estudiantes.length > 0 ? 'Elimina todos los docentes y estudiantes antes de eliminar el curso' : ''}>
                    🗑️ Eliminar Curso
                  </span>
                ),
                danger: true,
        ...
        <div className="card" style={{ overflow: 'hidden', marginTop: '1.5rem' }}>
            ...
            {loadingDetails ? (
              ...
              <Card className="section-tabs" bodyStyle={{ padding: 0 }} style={{ border: 'none', borderRadius: 0, boxShadow: 'none', marginTop: '0.75rem' }}>
                <Tabs type="card" activeKey={activeCursoTab} onChange={setActiveCursoTab} items={[
                  {
                    key: 'docentes',
      ...
      {false && selectedCurso && estudiantes.length > 0 && (
                    ...
                    <div style={{ 
                      ...
                      <p style={{ margin: 0, fontSize: '0.9rem', color: promedio >= 51 ? '#065f46' : '#991b1b', lineHeight: '1.5' }}>
                        <strong>📌 Estado:</strong> {promedio >= 51 
                          ? 'El estudiante ha aprobado el curso.' 
      ...
      {showAgregarDocenteModal && selectedCurso && docentesTaskGid && (
        <AgregarPersonaModal
          parentTaskGid={docentesTaskGid}
          tipo="Docente"
          parentName={selectedCurso.name}
      ...
      {showAgregarEstudianteModal && selectedCurso && estudiantesTaskGid && (
        <AgregarPersonaModal
      ...
      {showRegistroNotasModal && (
        <div className="modal-overlay">
          ...
          >
            <HtmlModalHeader icon="📝" title="Registro de Notas por Módulo" subtitle={selectedCurso?.name} onClose={() => setShowRegistroNotasModal(false)} />
      ...
      {showAsistenciaModal && (
        <div className="modal-overlay">
          ...
          >
            <HtmlModalHeader icon="✓" title="Registro de Asistencia" subtitle={selectedCurso?.name} onClose={() => setShowAsistenciaModal(false)} />
      ...
      {eliminarSeccionConfirm && selectedCurso && (
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '420px' }}>
            <div className="modal-header" style={{ borderBottom: '1px solid #fee2e2', backgroundColor: '#fff5f5' }}>
              <h2 style={{ fontSize: '1.1rem', color: '#dc2626', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div style={{ padding: '1.5rem' }}>
              <p style={{ marginBottom: '0.5rem', fontSize: '0.95rem', color: '#374151' }}>
                Estás a punto de eliminar el curso <strong>{selectedCurso.name}</strong> junto con todas sus tareas y subtareas.
              </p>
              <p style={{ marginBottom: '1.25rem', fontSize: '0.875rem', color: '#6b7280' }}>
