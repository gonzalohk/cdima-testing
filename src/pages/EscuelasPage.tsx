...
import { exportEscuelaGeneralPDF, exportEscuelaGeneralWord, exportEscuelaCentralizadorNotasPDF, exportEscuelaActaCalificacionesWord, exportEscuelaEstudiantePDF, exportEscuelaEstudianteWord, exportEscuelaNominaPDF, exportEscuelaNominaWord, exportEscuelaAsistenciaPDF, exportEscuelaAsistenciaWord, formatearNombreCompleto, parseInfoPrimariaLegacy } from '../services/reports/escuelas-reports.service';
...
interface DocLinkE { nombre: string; url: string; fecha?: string; }
const _getFileTypeE = (url: string): { icon: string; color: string; bg: string; label: string } => {
...
const _parseDocLinksE = (notes: string | undefined | null): DocLinkE[] => {
  if (!notes) return [];
...
const _buildDocNotesE = (originalNotes: string | undefined | null, documentos: DocLinkE[]): string => {
...
const EscuelasPage: React.FC = () => {
  ...
  const [eliminarPersonaTarget, setEliminarPersonaTarget] = useState<{ task: AsanaTask; tipo: 'Estudiante' } | null>(null);
  ...
  // Efecto para manejar escuela seleccionada desde el menú
  useEffect(() => {
    if (location.state?.selectedEscuela && escuelas.length > 0) {
      const escuela = escuelas.find(e => e.gid === location.state.selectedEscuela.gid);
      if (escuela) {
        handleViewDetails(escuela);
        // Limpiar el estado de navegación
        navigate(location.pathname, { replace: true, state: {} });
      }
    }
  }, [location.state, escuelas]);
  ...
  const handleEditEscuela = async () => {
    ...
    try {
      ...
      // Cargar área y número de módulos desde la tarea Resumen
      const tareaResumen = sectionTasks.find(t => t.name.startsWith('Resumen:'));
      let area = '';
      let numModulos = numeroModulos;
      let resumenTaskGid: string | undefined;

      if (tareaResumen) {
        resumenTaskGid = tareaResumen.gid;
        const tareaResumenFull = await asanaService.getTask(tareaResumen.gid);
        const areaField = tareaResumenFull.custom_fields?.find(
          (f: any) => f.name === ASANA_CUSTOM_FIELDS.AREA_ESCUELA
        );
        if (areaField?.enum_value?.name) area = areaField.enum_value.name;
        ...
        if (numMod !== null) numModulos = Math.min(10, Math.max(1, Math.round(numMod)));
      }
  ...
  const handleExportEscuelaGeneral = async () => {
    if (!selectedEscuela) return;
    try {
  ...
  const handleExportNominaPDF = () => {
    ...
    try {
      exportEscuelaNominaPDF({ escuela: selectedEscuela, estudiantes });
    } catch (error) {
  ...
  const handleExportNominaWord = () => {
    if (!selectedEscuela || estudiantes.length === 0) return;
    try {
      exportEscuelaNominaWord({ escuela: selectedEscuela, estudiantes });
    } catch (error) {
  ...
  const handleExportAsistenciaPDF = () => {
    if (!selectedEscuela || estudiantes.length === 0) return;
    try {
      exportEscuelaAsistenciaPDF({ escuela: selectedEscuela, estudiantes });
    } catch (error) {
  ...
  const handleExportAsistenciaWord = () => {
    if (!selectedEscuela || estudiantes.length === 0) return;
    try {
      exportEscuelaAsistenciaWord({ escuela: selectedEscuela, estudiantes });
    } catch (error) {
  ...
  */
  ...
  const handleExportActaCalificacionesWord = async () => {
    ...
    try {
      ...
      alert('Error al generar el acta. Por favor, intenta de nuevo.');
    }
  };
  ...
  const handleSaveDocumento = async () => {
    ...
    setDocModalError('');
    try {
      const existing = _parseDocLinksE(docModalSubtask.notes);
      ...
      const newNotes = _buildDocNotesE(docModalSubtask.notes, updated);
  ...
  const handleDeleteDocumento = async (subtask: AsanaTask, index: number) => {
    ...
    try {
      const existing = _parseDocLinksE(subtask.notes);
      const updated = existing.filter((_, i) => i !== index);
      const newNotes = _buildDocNotesE(subtask.notes, updated);
  ...
  const handleEliminarSeccion = async () => {
    ...
    try {
      ...
      await asanaService.deleteSection(selectedEscuela.gid);
      ...
      setNotification({ message: err instanceof Error ? err.message : 'Error al eliminar escuela', type: 'error' });
    } finally {
    ...
    <div className="planning-page">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.875rem 0', marginBottom: '0', gap: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', minWidth: 0, flex: 1 }}>
          ...
          <span style={{ fontSize: '1rem', color: '#64748b', fontWeight: 600, whiteSpace: 'nowrap', flexShrink: 0, letterSpacing: '-0.01em' }}>Escuela en:</span>
          <style>{`
            ...
            .esc-inline-sel .ant-select-selection-item { font-size: 1.2rem !important; font-weight: 700 !important; color: #1e3a5f !important; }
            .esc-inline-sel .ant-select-selection-placeholder { font-size: 1.1rem !important; color: #94a3b8 !important; }
          ...
          <Dropdown
            menu={{ items: [
              ...
              { type: 'divider' },
              {
                ...
                label: (
                  <span title={estudiantes.length > 0 ? 'Elimina todos los estudiantes antes de eliminar la escuela' : ''}>
                    🗑️ Eliminar Escuela
                  </span>
                ),
                danger: true,
                disabled: loadingDetails || estudiantes.length > 0,
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                {estudiantes.length > 0 && (
                  <Dropdown
                    menu={{ items: [
                      { key: 'pdf', label: '📄 Listado General PDF', onClick: handleExportEscuelaGeneral },
                      { key: 'word', label: '📝 Listado General Word', onClick: handleExportEscuelaGeneralWord },
                      { key: 'centralizador', label: '🗂 Centralizador de Notas PDF', onClick: handleExportCentralizadorNotas },
                      { key: 'acta', label: '📋 Acta de Calificaciones Word', onClick: handleExportActaCalificacionesWord },
                    ]}}
                    trigger={['click']}
                  >
                    <button
                        ...
                        {(() => {
                          ...
                          <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden' }}>
                            <style>{`
                              ...
                              .est-tbl-esc col.col-id   { width: 250px; }
                              .est-tbl-esc col.col-age  { width: 150px; }
                              .est-tbl-esc col.col-ori  { width: 140px; }
                              .est-tbl-esc col.col-act  { width: 100px; }
                              .est-tbl-esc col.col-rep  { width: 90px; }
                    ...
                    children: (
                      <div>
                        {(() => {
                          ...
                          const filtradosAsistencia = busquedaAsistencia
                            ? asistenciasPorEstudiante.filter(est => est.nombre.toLowerCase().includes(busquedaAsistencia.toLowerCase()))
                          ...
                          return (
                            <>
                              ...
                              <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden' }}>
                                <style>{`
                                  .asist-view-esc tbody tr { transition: background 0.1s; }
                                .asist-view-esc tbody tr:nth-child(even) { background: #f9fafb; }
                                .asist-view-esc tbody tr:hover { background: #f8fafc !important; }
                                .asist-view-esc tbody tr:hover td[data-asistio="true"] { background: #d1fae5 !important; color: #16a34a !important; }
                                .asist-view-esc tbody tr:hover td[data-asistio="false"] { background: #fee2e2 !important; color: #dc2626 !important; }
                                .asist-view-esc thead th { background: #f1f5f9; border-bottom: 2px solid #e2e8f0; }
                                .asist-view-esc .col-num-s  { position: sticky; left: 0;     z-index: 2; background: white; }
                                .asist-view-esc .col-name-s { position: sticky; left: 46px;  z-index: 2; background: white; box-shadow: 2px 0 5px -1px rgba(0,0,0,0.08); }
                                .asist-view-esc tbody tr:nth-child(even) .col-num-s,
                                .asist-view-esc tbody tr:nth-child(even) .col-name-s { background: #f9fafb; }
                                .asist-view-esc tbody tr:hover .col-num-s,
                                .asist-view-esc tbody tr:hover .col-name-s { background: #f8fafc !important; }
                                .asist-view-esc thead .col-num-s,
                                .asist-view-esc thead .col-name-s { z-index: 3; background: #f1f5f9; }
                              `}</style>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 0.875rem', backgroundColor: '#f8fafc', borderBottom: '1px solid #e5e7eb', gap: '0.5rem', flexWrap: 'wrap' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                  ...
                                  <button
                                    ...
                                    className="button-secondary"
                                    style={{ fontSize: '0.875rem', padding: '0.5rem 1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
                                  >🖨️ Asistencia PDF</button>
                                  <button
                                    onClick={handleExportAsistenciaWord}
                                    className="button-secondary"
                                    style={{ fontSize: '0.875rem', padding: '0.5rem 1rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
                                  >📄 Asistencia Word</button>
                                </div>
                              </div>
                              <div style={{ overflowX: 'auto' }}>
                              <table className="asist-view-esc" style={{ borderCollapse: 'collapse' }}>
                                <thead>
                                  <tr>
                                ...
                                <tbody>
                                  {filtradosAsistencia.map((est, eIdx) => (
                                    <tr key={eIdx}>
                        ...
                        {documentos.length === 0 ? (
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${documentos.length}, 1fr)`, gap: '1rem', alignItems: 'start' }}>
                            {documentos.map((documento, colIdx) => {
                              const docLinks = _parseDocLinksE(documento.notes);
                              const colPalette = [
                                ...
                                <div key={documento.gid} style={{ border: `1.5px solid ${col.border}`, borderRadius: '12px', overflow: 'hidden', background: 'white' }}>
                                  ...
                                  <div style={{ padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', minHeight: '60px' }}>
                                    {docLinks.length === 0 ? (
                                      ...
                                      docLinks.map((doc, i) => {
                                        const ft = _getFileTypeE(doc.url);
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              <button
                ...
                title="Exportar Acta de Calificaciones"
                style={{
                  background: 'none',
                ...
                return (
                  <div>
                    <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden', marginBottom: '1rem' }}>
                      <style>{`
                        .notas-tbl-esc tbody tr { transition: background 0.1s; }
                        .notas-tbl-esc tbody tr:nth-child(even) { background: #f9fafb; }
                        .notas-tbl-esc tbody tr:hover { background: #eff6ff !important; }
                        .notas-tbl-esc thead th { position: sticky; top: 0; z-index: 1; }
                      `}</style>
                      <table className="notas-tbl-esc" style={{ minWidth: '520px', width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                          <tr>
              ...
              {(() => {
                ...
                return (
                  <div>
                    ...
                    <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden' }}>
                      <style>{`
                        .asist-tbl-esc tbody tr { transition: background 0.1s; }
                        .asist-tbl-esc tbody tr:nth-child(even) { background: #f9fafb; }
                        .asist-tbl-esc tbody tr:hover { background: #f8fafc !important; }
                        .asist-tbl-esc tbody tr:hover td[data-asistio="true"] { background: #d1fae5 !important; color: #16a34a !important; }
                        .asist-tbl-esc tbody tr:hover td[data-asistio="false"] { background: #fee2e2 !important; color: #dc2626 !important; }
                        .asist-tbl-esc thead th { position: sticky; top: 0; z-index: 1; }
                      `}</style>
                      <table className="asist-tbl-esc" style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                          <tr>
      ...
      {selectedInfo && (
        <InfoPrimariaModal
          ...
          onSave={(gid, newName, newNotes) => {
            setEstudiantes(prev => prev.map(est => est.gid === gid ? { ...est, name: newName, notes: newNotes } : est));
      ...
      {showRegistroNotasModal && (
        <div className="modal-overlay">
          ...
          >
            <HtmlModalHeader icon="📝" title="Registro de Notas por Módulo" subtitle={selectedEscuela?.name} onClose={() => setShowRegistroNotasModal(false)} />
      ...
      {showAsistenciaModal && (
        <div className="modal-overlay">
          ...
          >
            <HtmlModalHeader icon="✓" title="Registro de Asistencia" subtitle={selectedEscuela?.name} onClose={() => setShowAsistenciaModal(false)} />
      ...
      {eliminarSeccionConfirm && selectedEscuela && (
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '420px' }}>
            <div className="modal-header" style={{ borderBottom: '1px solid #fee2e2', backgroundColor: '#fff5f5' }}>
              <h2 style={{ fontSize: '1.1rem', color: '#dc2626', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div style={{ padding: '1.5rem' }}>
              <p style={{ marginBottom: '0.5rem', fontSize: '0.95rem', color: '#374151' }}>
                Estás a punto de eliminar la escuela <strong>{selectedEscuela.name}</strong> junto con todas sus tareas y subtareas.
              </p>
              <p style={{ marginBottom: '1.25rem', fontSize: '0.875rem', color: '#6b7280' }}>
